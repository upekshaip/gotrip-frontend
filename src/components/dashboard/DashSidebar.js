...
export default function DashSidebar({
  ...
  useEffect(() => {
    const itemsWithRoutes = menuItems?.filter((item) => item.url);
    const active = itemsWithRoutes
      ?.sort((a, b) => b.url.length - a.url.length) // prioritize longest routes first
            ...
            menuItems.map(
              (item) =>
                item.menu && (
                  <li key={item.name}>
                    <button
                      className={`btn-wide transition flex border-0 rounded-r-lg
          ${
