...
const CameraBookingRequests = () => {
  ...
  const fetchBookings = useCallback(async (page) => {
    try {
      ...
      const response = await UseFetch(
        "GET",
        `/camera-booking/incoming-requests?page=${page}&limit=${limit}`,
      );
  ...
  const handleAction = async (bookingId, statusType) => {
    ...
    try {
      ...
      const response = await UseFetch(
        "PATCH",
        `/camera-booking/${bookingId}/respond`,
        payload,
      );
  ...
  return (
    <div className="space-y-4 pb-8 relative">
      {bookings.map((item, index) => {
        const { booking, cameraDetails, travellerInfo } = item;
...
export default CameraBookingRequests;
