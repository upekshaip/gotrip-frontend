...
export default function TransportManagementPage() {
  ...
  const [vehicleData, setVehicleData] = useState({
    ...
    imageUrl: "",
    isFeatured: false,
    status: "PENDING", // Default to PENDING when adding a new vehicle
    latitude: 1.5,
    longitude: 1.5,
  });
  ...
  const handleAction = async (bookingId, newStatus) => {
    ...
    try {
      ...
      if (res && !res.error) {
        ...
        setRequests((prev) =>
          prev.filter((req) => req.booking.bookingId !== bookingId),
        );
      } else {
    ...
    <div className="p-6 max-w-7xl mx-auto min-h-screen">
      ...
      <div className="mb-8">
        ...
        <p className="text-base-content/60 mt-1">
          Manage your fleet and incoming ride requests
        </p>
      </div>
                  ...
                  >
                    <div className="card-body p-6">
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <h3 className="font-bold text-lg">
                              {req.travellerContact?.name || "Traveler"}
                            </h3>
                            <p className="text-xs text-base-content/60 flex items-center gap-1">
                              <User size={12} />{" "}
                              {req.travellerContact?.email || "No email"}
                            </p>
                          </div>
                        </div>
                        <div className="text-right">
                          <span className="text-xl font-black text-primary">
                            LKR {req.booking.totalAmount}
                          </span>
                      ...
                      <div className="grid grid-cols-2 gap-4 mb-4 text-sm">
                        <div>
                          <p className="text-xs opacity-50 font-bold uppercase mb-1 flex items-center gap-1">
                            <Calendar size={12} /> Dates
                          </p>
                          <p className="font-medium">
                            {req.booking.startingDate} to{" "}
                            {req.booking.endingDate}
                          </p>
                        </div>
                        <div>
                          <p className="text-xs opacity-50 font-bold uppercase mb-1 flex items-center gap-1">
                          ...
                          <p className="font-medium">
                            {req.booking.startingTime} -{" "}
                            {req.booking.endingTime}
                          </p>
                        </div>
                      ...
                      <div className="card-actions justify-end mt-2 pt-4 border-t border-base-200 gap-3">
                        {req.booking.status === "PENDING" && (
                          <>
                            <button
                              onClick={() =>
                                handleAction(req.booking.bookingId, "DECLINED")
                              }
                            ...
                            <button
                              onClick={() =>
                                handleAction(req.booking.bookingId, "ACCEPTED")
                              }
                              disabled={processingId === req.booking.bookingId}
                              className="btn btn-primary flex-1 sm:flex-none"
                            >
                              {processingId === req.booking.bookingId ? (
                        ...
                        {req.booking.status === "ACCEPTED" && (
                          <button
                            onClick={async () => {
                              ...
                              try {
                                const res = await markBookingCompleted(
          ...
          <div className="card-body p-6 md:p-8">
            ...
            <form onSubmit={handleAddVehicle} className="space-y-5">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div className="form-control">
                  <label className="label">
                    <span className="label-text font-bold">Make</span>
                  </label>
                  <input
                    type="text"
                    ...
                    onChange={(e) =>
                      setVehicleData({
                        ...
                        status: "PENDING", // Set status to PENDING when adding a new vehicle
                        latitude: 1.5,
                        longitude: 1.5,
                      })
                    }
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div className="form-control">
                  <label className="label">
                    <span className="label-text font-bold">Vehicle Type</span>
                  </label>
                  <select
                ...
                <div className="form-control">
                  <label className="label">
                    <span className="label-text font-bold">City Location</span>
                  </label>
                  <input
                    type="text"
                    ...
                    onChange={(e) =>
                      setVehicleData({ ...vehicleData, city: e.target.value })
                    }
                  />
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
                ...
                <div className="form-control">
                  <label className="label">
                    <span className="label-text font-bold">Pricing Unit</span>
                  </label>
                  <select
                    className="select select-bordered w-full"
                    value={vehicleData.priceUnit}
                    onChange={(e) =>
                      setVehicleData({
                        ...vehicleData,
              ...
              <div className="form-control">
                ...
                <textarea
                  ...
                  placeholder="Describe the vehicle condition, AC availability, restrictions..."
