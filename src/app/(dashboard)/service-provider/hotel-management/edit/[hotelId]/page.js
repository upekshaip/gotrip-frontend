    ...
    const fetchHotel = async () => {
      ...
      try {
        const data = await UseFetch(
          "GET",
          `/hotel-service/${resolvedParams.hotelId}`,
        );

        if (data.timestamp || !data) {
          toast.error("Failed to get hotel information.");
        } else {
          setHotelData(data);
          setFormData({
            name: data.name || "",
            ...
            priceUnit: data.priceUnit || "PER_DAY",
            price: data.price || 0,
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (res.timestamp) {
        ...
        router.push(routes.serviceProvider.hotelManagement.url);
      }
    } catch (error) {
    ...
    return (
      <div className="alert alert-error mt-4">
        <div className="flex items-center gap-3">
          ...
          <span>Hotel not found with the provided ID.</span>
        </div>
      </div>
    );
  }
          ...
          <div className="form-control">
            <label className="label">
              <span className="label-text text-xs font-medium mb-2 flex items-center gap-2">
                <MapPin className="w-4 h-4" /> Edit Hotel Location (Sri Lanka
                Only)
              </span>
            </label>
            <MapSelector
