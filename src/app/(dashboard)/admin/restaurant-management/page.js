...
import AdminRestaurantManagement from "@/components/restaurant-management/AdminRestaurantManagement";
import PendingRestaurantsAdmin from "@/components/restaurant-management/PendingRestaurantsAdmin";

const RestaurantManagementPage = () => {
  const [activeTab, setActiveTab] = useState("all");

  const tabs = [
    { id: "all", label: "All", icon: <Coffee size={16} /> },
    {
      id: "pendingRequests",
      ...
      <div className="mx-auto px-4">
        ...
        <header className="flex flex-col md:flex-row md:items-center justify-between gap-8 mb-12">
          <div className="space-y-2">
            <h1 className="font-bold tracking-tight">Restaurant Management</h1>
          </div>
        ...
        <main className="min-h-[400px]">
          {activeTab === "all" && <AdminRestaurantManagement />}
          {activeTab === "pendingRequests" && <PendingRestaurantsAdmin />}
        </main>
      </div>
    </div>
  );
