...
const AllServiceProvidersPage = () => {
  ...
  const [page, setPage] = useState(1); // 🔹 0-indexed based on typical backend response
  ...
  const fetchProviders = async (isFirstLoad = false, nextPage = 1) => {
    if (loading) return;
    setLoading(true);
    ...
    try {
      const data = await UseFetch(
        "GET",
        `/user/admin/all-providers?${params.toString()}`,
      );
    ...
    (node) => {
      ...
      observer.current = new IntersectionObserver(async (entries) => {
        if (entries[0].isIntersecting && hasMore && !loading) {
          ...
          setPage(nextPage);
          await fetchProviders(false, nextPage);
        }
      });
      if (node) observer.current.observe(node);
    },
    [loading, hasMore, page, fetchProviders],
  );
  ...
  return (
    <section className="section-container">
        ...
        <div className="sticky top-0 p-4 bg-base-100 border-b border-base-200 z-10">
          <div className="flex flex-wrap items-center justify-between gap-4">
            ...
            <div className="badge badge-outline gap-2 p-3">
              Total Providers: {providers.length} Loaded
            </div>
          </div>
        </div>
      ...
      {/* MODAL 1: PROFILE EDITOR */}
      <div className={clsx("modal", selectedUser && "modal-open")}>
        <div className="modal-box max-w-md">
          <div className="flex items-center justify-between mb-6">
            <h3 className="font-bold text-lg flex items-center gap-2">
              <UserCircle size={20} className="text-primary" /> Edit Provider
              Profile
            </h3>
            <button
